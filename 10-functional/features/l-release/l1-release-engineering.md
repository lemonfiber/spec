...
| A feature `1.0.0` locks is Accepted but not yet built | `1.0.0` MUST NOT ship; the no-stub bar is unmet. |
...
| **L1-R10** | `1.0.0` MUST open the dashboard on a bare invocation — the capability v1 builds toward — and MUST NOT be cut while any feature it locks is not both Accepted and `shipped`. |
