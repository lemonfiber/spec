...
| `lemonfiber-plugins` | git tag `vX.Y.Z` | A signed catalogue release. **Not on the version train** (`OPS-R59`); nothing in `70-operations/versions/` names it |
...
Both are carried today: `cargo-dist` emits the attestation via GitHub's artifact
attestations, so a consumer can verify a downloaded binary came from the real
pipeline. It also raises the [OpenSSF Scorecard](../40-quality/tooling.md) signal
the project already publishes.

`OPS-R20` also obliges an **SBOM** (software bill of materials) per release, and
that half is not produced yet — answering "are we affected?" still means reading
`Cargo.lock` at the tag rather than a manifest of what went in.
...
is cut when the M10 exit criteria are met — a non-contributor can install and run
