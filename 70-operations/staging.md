...
| **OPS-R54** | A version MUST NOT be released while a feature it locks — one whose requirements its `goals` name — is neither `maturity: built` nor `maturity: shipped` in the feature catalogue, and a refusal MUST name those features. A major ships no stubs, and neither does any version before it. `built` is the state that makes this checkable: `shipped` means out in a released version, so requiring it before release would be a gate no version could ever pass. |
