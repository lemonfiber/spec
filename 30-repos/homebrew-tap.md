...
homebrew-tap/
├── Formula/
│   └── lemonfiber.rb        a placeholder until release CI generates it
├── scripts/check_formula.py `brew style` and `ruby -c` over the formula
├── justfile
└── README.md
```
...
2. Once generated, the formula's `license` line is a non-standard string; the
   committed placeholder has none. `brew audit --strict` will warn,
...
When lemonfiber was installed via Homebrew, `lemonfiber update self` **defers**
...
| **REPO-R27** | The tap MUST stay current so `lemonfiber update self`'s Homebrew deferral has a valid target. |
